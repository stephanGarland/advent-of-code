...
class Solution:
    ...
    # This is just straight-up https://www.reddit.com/r/adventofcode/comments/zgnice/2022_day_9_solutions/j0cw2py/
        ...
        for direction, magnitude in input_data:
            for _ in range(magnitude):
                ...
                for knot in range(1, len(rope)):
                    ...
                    if abs(diff) > SQRT_2:
                        if diff.real != 0:
                            rope[knot] += diff.real / abs(diff.real)
                        if diff.imag != 0:
                            rope[knot] += complex(0, diff.imag) / abs(diff.imag)
                visited.add(rope[-1])
